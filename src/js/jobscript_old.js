...
function createEstablishmentPopup(establishment) {
    ...
    // Add salary info
    establishmentContent.appendChild(createInfoRow('時薪', establishment.salary));
    
    // Add boolean values with icons
    establishmentContent.appendChild(createBooleanRow('供餐', establishment.供餐));
    establishmentContent.appendChild(createBooleanRow('試用期', establishment.試用期));
    establishmentContent.appendChild(createBooleanRow('勞健保', establishment.勞健保));
    establishmentContent.appendChild(createBooleanRow('國定雙倍', establishment.國定雙倍));
    
    // Add star ratings
    establishmentContent.appendChild(createStarRatingRow('環境評分', establishment.環境評分, establishment.環境評分人數));
    establishmentContent.appendChild(createStarRatingRow('滿意度評分', establishment.滿意度評分, establishment.滿意度評分人數));
    ...
    explanationText.innerHTML = '依據勞動基準法規定，<br>以下是雇主應遵守的基本標準：';
    ...
    standardsContent.appendChild(createComparisonRow('勞健保', window.legalStandard.勞健保, establishment.勞健保));
    standardsContent.appendChild(createComparisonRow('國定雙倍', window.legalStandard.國定雙倍, establishment.國定雙倍));
...
function createBooleanRow(label, value) {
    const row = document.createElement('div');
    row.className = 'info-row-job';
    const iconClass = value ? 'fas fa-check yes' : 'fas fa-times no';
    const displayValue = value ? '是' : '否';
    row.innerHTML = `
        <div class="label">${label}</div>
        <div class="value"><i class="${iconClass}"></i> ${displayValue}</div>
    `;
    return row;
}

function createStarRatingRow(label, rating, raterCount) {
    const row = document.createElement('div');
    row.className = 'info-row-job';
    ...
    row.innerHTML = `
        ...
        <div class="value">
            ${starsHtml}
            <span class="rater-count">(${raterCount || 0}人)</span>
        </div>
    `;
    return row;
}

function createComparisonRow(label, standard, actual) {
    const row = document.createElement('div');
    row.className = 'comparison-row-job';
    ...
    if (label === '時薪') {
        ...
        const meetsStandard = actualNum >= standardNum;
        comparisonHtml = `
            <div class="label">${label}</div>
            <div class="value">
                <div>法定: ${standard}</div>
                <div>實際: ${actual} ${meetsStandard ? '<i class="fas fa-check yes"></i>' : '<i class="fas fa-times no"></i>'}</div>
            </div>
        `;
    } else {
        // Boolean comparison
        const meetsStandard = (standard === true || standard === '是') === (actual === true || actual === '是');
        comparisonHtml = `
            <div class="label">${label}</div>
            <div class="value">
                <div>法定: ${standard === true || standard === '是' ? '是' : '否'}</div>
                <div>實際: ${actual ? '是' : '否'} ${meetsStandard ? '<i class="fas fa-check yes"></i>' : '<i class="fas fa-times no"></i>'}</div>
            </div>
        `;
    }
    
    row.innerHTML = comparisonHtml;
    return row;
}
...
function setupMobilePopup() {
    ...
    window.showMobilePopup = function(marker) {
        ...
        // Add all establishment info
        establishmentInfoContent.appendChild(createInfoRow('時薪', marker.establishment.salary));
        establishmentInfoContent.appendChild(createBooleanRow('供餐', marker.establishment.供餐));
        establishmentInfoContent.appendChild(createBooleanRow('試用期', marker.establishment.試用期));
        establishmentInfoContent.appendChild(createBooleanRow('勞健保', marker.establishment.勞健保));
        establishmentInfoContent.appendChild(createBooleanRow('國定雙倍', marker.establishment.國定雙倍));
        establishmentInfoContent.appendChild(createStarRatingRow('環境評分', marker.establishment.環境評分, marker.establishment.環境評分人數));
        establishmentInfoContent.appendChild(createStarRatingRow('滿意度評分', marker.establishment.滿意度評分, marker.establishment.滿意度評分人數));
        ...
        // Add comparison rows
        legalStandardsContent.appendChild(createComparisonRow('時薪', window.legalStandard.salary, marker.establishment.salary));
        legalStandardsContent.appendChild(createComparisonRow('勞健保', window.legalStandard.勞健保, marker.establishment.勞健保));
        legalStandardsContent.appendChild(createComparisonRow('國定雙倍', window.legalStandard.國定雙倍, marker.establishment.國定雙倍));
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function applySearch() {
        try {
            ...
            const legalCompliance = document.getElementById('legal-compliance')?.checked || false;
    ...
    function filterEstablishments(params) {
        ...
        if (window.originalMarkers) {
            ...
            window.originalMarkers.forEach(marker => {
                ...
                // Legal compliance filter - check all legal requirements
                if (params.filters.legalCompliance) {
                    const meetsLegalStandards = 
                        establishment.hourlyWage >= 180 && // Assuming 180 is minimum wage
                        establishment.勞健保 === true &&
                        establishment.國定雙倍 === true;
                    
                    if (!meetsLegalStandards) {
